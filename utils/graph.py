...
class Graph():
    ...
    def popLandmark(self):
        if len(self.landmarks) == 0:
            print('Current landmarks list is empty.')
            return False
        self.landmarks.pop()
    ...
    def popNode(self):
        self.revertEdges()
        self.current_node = len(self.nodes) - 2 if len(self.nodes) > 2 else 0
        self.nodes.pop()
    ...
    def save(self, save_path):
        ...
        if self.goal_location is not None:
            data['goal_location'] = None if self.goal_location is None else self.goal_location.tolist()
        else:
            data['goal_location'] = None
        data['start_location'] = None if self.nodes[0].location is None else self.nodes[0].location.tolist()
...
class Node():
    ...
    def getSaveDict(self):
        return {'img_path': self.img_path,
                ...
                'location': None if self.location is None else self.location.tolist() ,
                'orientation': None if self.orientation is None else self.orientation.angle(),
                'click_point': None if self.click_point is None else self.click_point.tolist(),
                'reason': None if self.reason is None else self.reason}
