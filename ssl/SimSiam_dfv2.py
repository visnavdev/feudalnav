# https://docs.lightly.ai/self-supervised-learning/examples/simsiam.html
import os
import torch
import torchvision
...
save_pred_path = f'{save_path}/pred' # prediction
if not os.path.exists(save_pred_path): os.makedirs(save_pred_path)
...
for epoch in range(epochs):
    ...
    save_pred_file_path = f'{save_pred_path}/pred_epoch_{epoch}.pth' # prediction
    ...
    save_pred_best_file_path = f'{save_pred_path}/pred_best.pth' # prediction
    ##########################
    ...
    if epoch % epochs == 0:
        ...
        torch.save(model.prediction_head.state_dict(), save_pred_file_path); print(f'{save_pred_file_path} saved!')
    if avg_loss < curr_loss:
        ...
        torch.save(model.prediction_head.state_dict(), save_pred_best_file_path); print(f'{save_pred_best_file_path} saved!')
        curr_loss = avg_loss
    ...
    model.prediction_head.load_state_dict(torch.load(save_pred_file_path)); print(f'{save_pred_file_path} loaded!')
    '''
