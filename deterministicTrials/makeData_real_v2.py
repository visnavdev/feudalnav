'''
Usage:
python deterministicTrials/makeData_real.py --save_env_name --env_name 480p_scene01
'''
...
def getArgs():
    ...
    parser.add_argument('--data_path', type=str, default='/aw2data/Data/datasets/LMNav/480p',
                        help='path to the folder with the dataset graph data') # edit
...
if __name__ == '__main__':
    ...
    while frame_i < len(image_paths):
        image_path = image_paths[frame_i]
        ...
        #     print('Down arrow')
            ...
            action = 'undo'; print(action)

        if action == 'undo':
            # Reverse the steps for adding data into the graph
            worker.graph.popLandmark()
            if len(worker.graph.actions) > 0:
                worker.graph.actions.pop()
                worker.graph.popNode()
            frame_i -= 1
            continue
        else:
